...
def extract_residence_fields(ocr_text):
    ...
    date_match = re.search(r'(?:statement date|bill date|date of issue|issued date|date)[:\s]*([0-9]{2,4}[-/.][0-9]{1,2}[-/.][0-9]{2,4}|[0-9]{1,2}\s+[a-z]{3,9}\s+[0-9]{4})', text_lower)
    if date_match:
        ...
        dates = re.findall(r'\b(202[0-9])[-/.](0[1-9]|1[0-2])[-/.](0[1-9]|[12]\d|3[01])\b', ocr_text)
        if dates:
    ...
    acc_match = re.search(r'(?:account no|account number|a/c no|bill no|invoice no|statement no)[:\s]*([a-z0-9\-]{5,20})', text_lower)

    if aadhaar_match:
        extracted['doc_number'] = f"{aadhaar_match.group(1)}{aadhaar_match.group(2)}{aadhaar_match.group(3)}"
    elif pan_match:
    ...
    name_match = re.search(r'(?:customer name|name|billed to|account holder|holder)[:\s]*([a-zA-Z\s]{3,40})', ocr_text, re.IGNORECASE)
    if name_match:
    ...
    address_lines = []
    header_kws = ['identification', 'authority', 'government of india', 'help@uidai', 'print date', 'www.uidai']
    for line in lines:
        l_low = line.lower()
        if not any(h_kw in l_low for h_kw in header_kws):
            if any(kw in l_low for kw in ['street', 'road', 'avenue', 'block', 'lane', 'nagar', 'city', 'state', 'pincode', 'zip', 'post', 'flat', 'apt', 'address:']):
...
def verify_residence_document(user, document_instance, file_path, ocr_text, quality_metrics):
        ...
        try:
            ...
            for id_doc in id_docs:
                ...
                if id_num:
                    ...
                    if clean_id_num and (clean_id_num == clean_res_num or clean_id_num in clean_raw_text or (len(clean_id_num) >= 4 and clean_id_num[-4:] in clean_raw_text)):
    ...
    if doc_name and any(kw in doc_name.lower() for kw in ['psa', 's/o', 'd/o', 'w/o', 'c/o', 'government', 'authority', 'india']):
        doc_name = None
    ...
    if doc_number_matched_id_proof:
        ...
        if name_score < 45.0:
            warnings.append(f"Name mismatch on residence proof: Profile '{profile_full_name}' vs Document Name ('{doc_name}').")
    else:
        text_lower = ocr_text.lower() if ocr_text else ''
        is_relationship_addr = any(kw in text_lower for kw in ['s/o', 'd/o', 'w/o', 'c/o', 'son of', 'daughter of', 'wife of', 'care of', 'address:'])
        if is_relationship_addr:
            name_score = 85.0
        else:
            name_score = 30.0
    ...
    profile_address_parts = [str(v).strip() for v in profile_address_dict.values() if v and str(v).strip()]
    profile_address_str = ", ".join(profile_address_parts)

    if not profile_address_parts:
        address_score = 60.0
        warnings.append("Registered profile address is incomplete. Please update your profile address.")
    else:
        ...
        if address_score < 40.0:
            warnings.append(f"Address mismatch on residence proof: Profile address ('{profile_address_str}') does not match the uploaded document.")

    # 3. Statement Date / Validity Score
    date_validity_score = 90.0
    if not extracted.get('is_recent_statement'):
        date_validity_score = 40.0
        warnings.append(f"Statement date ({extracted.get('statement_date')}) appears older than 180 days.")
