...
IGNORED_DOC_NUMBER_WORDS = {
    'DEPARTMENT', 'GOVERNMENT', 'PERMANENT', 'NATIONAL', 'REPUBLIC',
    'IDENTITY', 'DOCUMENT', 'PASSPORT', 'LICENCE', 'LICENSE', 'CARD', 'INDIA',
    'EMIRATES', 'KINGDOM', 'UNITED', 'STATES', 'AMERICA', 'CANADA', 'AUSTRALIA'
}
...
def extract_identity_fields(ocr_text, user_profile_name=None):
    ...
    if mrz_p1 and mrz_p2:
        ...
        parts = [p.replace('<', ' ').strip() for p in name_str.split('<<') if p.replace('<', ' ').strip()]
        if parts:
            if len(parts) >= 2:
    ...
    if extracted['doc_subtype'] == 'identity_document':
        if 'passport' in text_lower or 'passeport' in text_lower or 'pasaporte' in text_lower or 'reise pass' in text_lower:
            extracted['doc_subtype'] = 'passport'
        elif any(kw in text_lower for kw in ['driver', 'driving', 'licence', 'license', 'permis', 'permiso', 'führerschein']):
            extracted['doc_subtype'] = 'driver_license'
        elif 'emirates' in text_lower or '784-' in text_lower:
            extracted['doc_subtype'] = 'emirates_id'
        elif 'iqama' in text_lower or 'residence permit' in text_lower or 'green card' in text_lower:
            extracted['doc_subtype'] = 'residence_permit'
        elif 'aadhaar' in text_lower or 'uidai' in text_lower:
            extracted['doc_subtype'] = 'aadhaar'
        elif 'pan' in text_lower or 'permanent account' in text_lower or 'income tax' in text_lower:
            extracted['doc_subtype'] = 'pan_card'
        elif 'voter' in text_lower or 'election' in text_lower:
            extracted['doc_subtype'] = 'voter_id'
        elif 'national' in text_lower or 'republic' in text_lower or 'republique' in text_lower or 'state id' in text_lower:
            extracted['doc_subtype'] = 'national_id'

    # Date of Birth (DOB) Extraction if not set by MRZ
    if not extracted['dob']:
        dob_match = re.search(r'(?:dob|date of birth|birth date|bday|born|date de naissance|fecha de nacimiento)[:\s]*([0-9]{1,4}[-/.\s][0-9]{1,2}[-/.\s][0-9]{2,4}|[0-9]{1,2}\s+[a-z]{3,9}\s+[0-9]{4})', text_lower)
        if dob_match:
            extracted['dob'] = dob_match.group(1).strip()
        else:
            dates_ymd = re.findall(r'\b(19\d\d|20[0-2]\d)[-/.](0[1-9]|1[0-2])[-/.](0[1-9]|[12]\d|3[01])\b', ocr_text)
            dates_dmy = re.findall(r'\b(0[1-9]|[12]\d|3[01])[-/.](0[1-9]|1[0-2])[-/.](19\d\d|20[0-2]\d)\b', ocr_text)
            if dates_ymd:
    ...
    if not extracted['expiry_date']:
        exp_match = re.search(r'(?:exp|expiry|valid till|valid until|expires|exp date|date d\'expiration)[:\s]*([0-9]{2,4}[-/.][0-9]{1,2}[-/.][0-9]{2,4}|[0-9]{1,2}\s+[a-z]{3,9}\s+[0-9]{4})', text_lower)
        if exp_match:
    ...
    if not extracted['doc_number']:
        for pattern in DOC_NUMBER_PATTERNS:
            ...
            for m in matches:
                ...
                if not clean_m.startswith('19') and not clean_m.startswith('20') and len(clean_m) >= 6:
                    if clean_m not in IGNORED_DOC_NUMBER_WORDS and any(char.isdigit() for char in clean_m):
    ...
    candidates = []
    ignore_keywords = [
        'republic', 'government', 'passport', 'license', 'licence', 'dob', 'birth',
        'date', 'department', 'income tax', 'permanent account', 'number', 'govt',
        'india', 'signature', 'father', 'card'
    ]

    # Regex matches first
    name_matches = re.findall(r'(?:name|full name|given name)[:\s]*([a-zA-Z\s\.]{3,40})', ocr_text, re.IGNORECASE)
    ...
    for line in lines:
        ...
        if not any(kw in l_lower for kw in ignore_keywords):
            ...
            if len(clean_line) >= 4 and not any(char.isdigit() for char in clean_line):
                candidates.append(clean_line)

    if candidates:
        if user_profile_name:
            best_candidate = max(candidates, key=lambda c: compare_names(user_profile_name, c))
            extracted['full_name'] = best_candidate
        else:
            extracted['full_name'] = candidates[0]
...
def verify_identity_document(user, document_instance, file_path, ocr_text, quality_metrics):
    ...
    """
    warnings = []
    profile_full_name = f"{getattr(user, 'first_name', '')} {getattr(user, 'last_name', '')}".strip()
    if not profile_full_name:
        profile_full_name = getattr(user, 'username', '') or getattr(user, 'email', '')
