...
def classify_document(ocr_text, expected_doc_type):
    ...
    logger.info(
        f"Document classification scores for expected '{expected_doc_type}': identity={identity_score}, residence={residence_score}, invalid={invalid_score}"
    )
    ...
    if invalid_score >= 2 and identity_score < 2 and residence_score < 2:
        return (
            ...
            "unrelated_document",
            85.0,
            "Uploaded document appears to be an invoice, academic certificate, or unrelated file.",
        )
    ...
    if expected == "identity":
        if detected_type == "residence" and residence_score >= 2 and identity_score == 0:
            return (
                ...
                "residence",
                confidence,
                "Document uploaded as Identity appears to be a Proof of Residence / Bank Statement.",
            )
        if invalid_score >= 2:
            return (
                False,
                "invalid",
                confidence,
                "Document uploaded as Identity is not a valid government identity proof.",
            )
        if identity_score >= 1:
        ...
        return (
            ...
            detected_type,
            40.0,
            f"Uploaded document text does not match expected '{expected}' document format.",
        )
        ...
        if detected_type == "identity" and identity_score >= 2 and residence_score == 0:
            return (
                ...
                "identity",
                confidence,
                "Document uploaded as Residence appears to be an Identity card / Passport.",
            )
        if invalid_score >= 2:
            return (
                False,
                "invalid",
                confidence,
                "Document uploaded as Residence is not a valid utility bill / bank statement proof.",
            )
