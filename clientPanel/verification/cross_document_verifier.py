...
def verify_cross_documents(user):
    ...
    identity_doc = UserDocument.objects.filter(user=user, document_type='identity').order_by('-uploaded_at', '-id').first()
    residence_doc = UserDocument.objects.filter(user=user, document_type='residence').order_by('-uploaded_at', '-id').first()

    if not identity_doc or not residence_doc:
        result['explanation'] = "Cross-document verification pending: Upload both Identity and Residence documents."
        return result
    ...
    if identity_doc.file_hash and residence_doc.file_hash and identity_doc.file_hash == residence_doc.file_hash:
    ...
    res_raw_text = (residence_doc.extracted_data or {}).get('raw_text', '') or (residence_doc.extracted_data or {}).get('extracted_address', '') or ''
    ...
    if id_numbers and res_numbers and (id_numbers & res_numbers):
        ...
        if c1 == c2 or (len(c1) >= 4 and len(c2) >= 4 and (c1[-4:] == c2[-4:] or c1 in c2 or c2 in c1)):
            same_doc_card = True
    ...
    if same_doc_card:
        ...
        result['explanation'] = "Cross-document verification successful: Both files belong to the exact same document (Document Number match)."
        return result

    # 4. Check for Address-side / Relationship Line (S/O, D/O, W/O, C/O, Address:)
    res_lower = res_raw_text.lower()
    is_address_side_doc = any(kw in res_lower for kw in ['s/o', 'd/o', 'w/o', 'c/o', 'son of', 'daughter of', 'wife of', 'care of', 'address:'])
    ...
    if target_name and res_name:
        ...
        if any(kw in res_name.lower() for kw in ['government of india', 'government', 'republic', 'department', 'authority']):
            res_name = None
    ...
    if target_name and res_name:
        ...
        if name_sim < 65.0:
            if is_address_side_doc and compare_names(user_profile_name, target_name) >= 40.0:
                ...
                result['explanation'] = "Cross-document verification successful: Identity document matches profile user, and Residence document provides valid address proof."
                return result

            result['is_consistent'] = False
            result['cross_match_score'] = min(result['cross_match_score'], round(float(name_sim), 1))
    ...
    if id_dob and res_dob:
        ...
        if not is_match:
            result['is_consistent'] = False
            result['cross_match_score'] = min(result['cross_match_score'], round(float(dob_score), 1))
    ...
    if not result['is_consistent']:
        ...
        if res_check_name and any(kw in res_check_name.lower() for kw in ['government of india', 'government', 'republic', 'department', 'authority']):
            res_check_name = None
        ...
        result['explanation'] = f"Cross-document verification successful: Both documents match (Name similarity {result['name_similarity']}%)."
