...
def check_document_quality(file_path):
    ...
    if width < QUALITY_THRESHOLDS['MIN_WIDTH'] or height < QUALITY_THRESHOLDS['MIN_HEIGHT']:
        result['quality_errors'].append(f"Image dimensions ({width}x{height}) are below required minimum ({QUALITY_THRESHOLDS['MIN_WIDTH']}x{QUALITY_THRESHOLDS['MIN_HEIGHT']}).")
    ...
    try:
        ...
        if std_dev < QUALITY_THRESHOLDS['MIN_COLOR_VARIANCE']:
            ...
            result['quality_errors'].append("Image is completely blank, solid color, or lacks detail.")
        ...
        if lap_var < QUALITY_THRESHOLDS['BLUR_LAPLACIAN_VAR_MIN']:
            result['is_blur'] = True
            result['quality_errors'].append(f"Document image is excessively blurry (blur score {round(lap_var, 1)} < {QUALITY_THRESHOLDS['BLUR_LAPLACIAN_VAR_MIN']}).")

    except Exception as e:
        logger.warning(f"OpenCV quality analysis error on {file_path}: {e}")

    # Final Quality Determination
    if not result['is_corrupt'] and not result['is_blank'] and not result['is_blur'] and len(result['quality_errors']) == 0:
        result['is_quality_sufficient'] = True
    ...
    try:
        ...
        if PYTESSERACT_AVAILABLE:
            try:
                ...
                logger.warning("Tesseract OCR binary not installed on Windows host. Please run C:\\VT-Index_CRM\\tesseract-setup.exe to enable automatic text extraction.")
