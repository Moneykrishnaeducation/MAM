...
def evaluate_verification_decision(
    document_type, scores_dict, quality_metrics, classification_result, fraud_result, warnings
):
    """
    ...
    if not quality_metrics.get("is_quality_sufficient"):
        decision = "rejected"
        reason = (
            f"document_quality_insufficient: {', '.join(quality_metrics.get('quality_errors', []))}"
        )
        return decision, confidence_int, reason, errors
...
def format_human_readable_reason(reason, errors=None, user_name=None, decision=None):
    ...
    if errors and len(errors) > 0:
        clean_err = (
            str(errors[0]).replace("major_name_mismatch: ", "").replace("major_dob_mismatch: ", "")
        )
