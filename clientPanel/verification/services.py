...
def trigger_document_verification(
    document_id, async_mode=True, performed_by_user=None, ip_address=None
):
    """
    ...
    if async_mode:
        try:
            ...
            task_res = verify_user_document_task.apply_async(
                args=[document_id, user_id, ip_address], retry=False
            )
            return {
            ...
            logger.warning(
                f"Async Celery/Redis dispatch skipped for doc #{document_id} ({e}). Executing verification synchronously."
            )

    # Synchronous processing fallback
    return process_document_verification(
        document_id=document_id, performed_by_user=performed_by_user, ip_address=ip_address
    )
