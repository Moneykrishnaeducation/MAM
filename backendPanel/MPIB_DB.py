...
def run_mam_script():
    ...
    class PositionSink:
                ...
                try:
                    for attempt in range(1, max_retries + 1):
                        ...
                        if success:
                            ...
                            try:
                                master_login_part = comment.split('_')[0] if comment else None
                                if master_login_part and master_login_part.isdigit():
                                    master_login = int(master_login_part)
                                    COPY_EXECUTOR.submit(self.handle_profit_share_async, master_login, follower_id, pos_ticket)
                            except Exception as ex:
                                logger.error(f"Failed to submit profit share async: {ex}")
        ...
        def process_profit_share(self, master_login, follower_id, closed_deal):
            try:
