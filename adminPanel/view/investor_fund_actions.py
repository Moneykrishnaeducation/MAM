"""Dedicated views for Investor financial actions: deposit, withdrawal, credit-in, credit-out, equity, history."""
...
async def investor_deposit_api(request):
    """Process Investor manual deposit via MT5 and sync DB."""
    return await _process_investor_financial_action(request, action_type="deposit")


@csrf_exempt
@permission_required(IsAdmin)
@require_http_methods(["POST"])
async def investor_withdraw_api(request):
    """Process Investor withdrawal via MT5 and sync DB."""
    return await _process_investor_financial_action(request, action_type="withdraw")


@csrf_exempt
@permission_required(IsAdmin)
@require_http_methods(["POST"])
async def investor_credit_in_api(request):
    """Process Investor credit-in (bonus) via MT5 and sync DB."""
    return await _process_investor_financial_action(request, action_type="credit-in")


@csrf_exempt
@permission_required(IsAdmin)
@require_http_methods(["POST"])
async def investor_credit_out_api(request):
    """Process Investor credit-out (deduction) via MT5 and sync DB."""
    return await _process_investor_financial_action(request, action_type="credit-out")
...
async def _process_investor_financial_action(request, action_type: str):
    ...
    # Log to ClientTransaction
    user_obj = trading_acc.user if trading_acc.user else None
    user_email = user_obj.email if user_obj else None
    user_role = user_obj.role if user_obj else "Investor"
