      ...
      <div className={`relative w-full max-w-2xl max-h-[90vh] md:max-h-none rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-white/10 ${isDarkMode ? 'bg-[#111b3d]' : 'bg-white'} flex flex-col`}>
        ...
        <div className="p-6 md:p-10 overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-6 md:mb-8">
            <div>
              <h2 className={`text-xl md:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-black'}`}>Establish MAM</h2>
              <p className={`text-xs md:text-xs mt-1 font-bold ${isDarkMode ? 'text-gray-400' : 'text-blue-600'}`}>Create a new master trading node</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            <div className="sm:col-span-2 space-y-2">
              <label className={`text-xs md:text-xs font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-gray-400' : 'text-black'}`}>Account Name</label>
              <input
                type="text" id="account_name" value={form.account_name} onChange={handleChange} required
                className={`w-full px-4 py-2.5 md:py-3 rounded-xl md:rounded-2xl border ${isDarkMode ? "bg-[#1e293b] border-blue-500/30 text-blue-400" : "bg-sky-50 border-black/10 text-black"} outline-none focus:ring-2 ring-blue-500/20 transition-all font-bold text-xs md:text-sm`}
                placeholder="e.g., Global Alpha MAM"
              />
            </div>

            <div className="space-y-2">
              <label className={`text-xs md:text-xs font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-gray-400' : 'text-black'}`}>Profit Share (%)</label>
              <input
            ...
            <div className="space-y-2">
              <label className={`text-xs md:text-xs font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-gray-400' : 'text-black'}`}>Risk Level</label>
              <select
            ...
            <div className="space-y-2">
              <label className={`text-xs md:text-xs font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-gray-400' : 'text-black'}`}>Leverage</label>
              <select
            ...
            <div className="space-y-2">
              <label className={`text-xs md:text-xs font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-gray-400' : 'text-black'}`}>Payout Frequency</label>
              <select
            ...
            <div className="sm:col-span-2 flex flex-col-reverse sm:flex-row gap-3 md:gap-4 pt-4 md:pt-6">
              <button type="button" onClick={() => setShowModal(false)} className={`w-full sm:flex-1 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-xs md:text-sm transition-all border ${isDarkMode ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-white border-[#1e3a8a]/10 text-blue-900 hover:bg-gray-50'}`}>Dismiss</button>
