        ...
        <div className="p-6 md:p-8 z-10 flex-1">
          ...
          <div className="mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold mb-2">
              <Sparkles size={13} className="animate-pulse" /> Student Center
            </div>
              ...
              <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-2xl">
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-xs">
                      <p className="font-medium text-slate-200">Two-Factor Auth</p>
                      <p className="text-slate-400">Enabled via Authenticator App</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-2xl">
                ...
                <div className="flex border-b border-slate-800 gap-6 mb-6 overflow-x-auto pb-1 scrollbar-hide">
                  ...
                  >
                    <Sliders size={15} /> MAM Preferences
                  </button>
                  <button 
                ...
                {activeTab === 'security' && (
                  <form onSubmit={triggerSaveToast} className="space-y-6">
                    ...
                    <div className="pt-4 border-t border-slate-800/80">
                      <h4 className="text-xs font-bold text-slate-100 mb-2">Two-Factor Authentication</h4>
                      <p className="text-xs text-slate-400 mb-4">Adding an extra layer of security reduces the risk of unauthorized login dramatically.</p>
                      <button type="button" className="px-4 py-2 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-colors rounded-xl text-xs font-semibold">
                ...
                {activeTab === 'preferences' && (
                  <form onSubmit={triggerSaveToast} className="space-y-6">
                    <div>
                      <h3 className="font-bold text-sm text-slate-100 mb-2">MAM Risk Management</h3>
                      <p className="text-xs text-slate-400 mb-6">Fine-tune risk metrics and allocations configured for the active master funds you trace.</p>

                      <div className="space-y-5">
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-xs font-medium text-slate-300">Target Risk Tolerance (Max Drawdown)</span>
                            <span className="text-xs font-bold text-emerald-400">{riskTolerance}% Limit</span>
                          </div>
                          <input 
                            type="range" 
                            min="10" 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                          <div>
                            <span className="text-xs font-semibold text-slate-300 block mb-2">Investment Strategy Goal</span>
                            <select className="w-full bg-slate-800 border border-slate-700/80 rounded-xl px-3 py-2.5 text-slate-200 text-xs focus:border-emerald-500 outline-none">
                              <option>Moderate Capital Appreciation</option>
                              <option>High Growth & Trading Revenue</option>
                              <option>Dynamic Volatility Hedging</option>
                              <option>Passive Index Overlay</option>
                            </select>
                          </div>
                          <div>
                            <span className="text-xs font-semibold text-slate-300 block mb-2">Leverage Scaling Multiplier</span>
                            <select className="w-full bg-slate-800 border border-slate-700/80 rounded-xl px-3 py-2.5 text-slate-200 text-xs focus:border-emerald-500 outline-none">
                              <option>1.0x (Standard Multiplier)</option>
                              <option>0.5x (Half Scale / Risk Off)</option>
                              <option>1.5x (Leveraged Boost)</option>
                              <option>2.0x (Double Allocation Scale)</option>
                            </select>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="pt-4 border-t border-slate-800/80 space-y-3">
                      <h4 className="text-xs font-bold text-slate-100">System Notification Alerts</h4>
                      <div className="space-y-2 text-xs">
                        <label className="flex items-center justify-between cursor-pointer py-1">
                          <span className="text-slate-300">New trade execution summary</span>
                          <input type="checkbox" defaultChecked className="toggle rounded accent-emerald-500" />
                        </label>
                        <label className="flex items-center justify-between cursor-pointer py-1">
                          <span className="text-slate-300">Risk drawdown triggers or alerts</span>
                          <input type="checkbox" defaultChecked className="toggle rounded accent-emerald-500" />
                        </label>
                      </div>
                    </div>
