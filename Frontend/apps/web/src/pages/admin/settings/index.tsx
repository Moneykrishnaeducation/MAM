...
const FormSkeleton = () => (
  <div className="animate-pulse space-y-6">
    <div className="h-4 bg-[#123283] rounded-full w-1/4 mb-2" />
    <div className="h-14 bg-[#081d5f] rounded-2xl w-full border border-[#2450b7]" />
    <div className="h-4 bg-[#123283] rounded-full w-1/4 mb-2" />
    <div className="h-14 bg-[#081d5f] rounded-2xl w-full border border-[#2450b7]" />
    <div className="h-4 bg-[#123283] rounded-full w-1/4 mb-2" />
    <div className="h-14 bg-[#081d5f] rounded-2xl w-full border border-[#2450b7]" />
    <div className="h-12 bg-[#123283] rounded-2xl w-full mt-6" />
  </div>
);
  ...
  return (
    <div className="space-y-2 group">
      <div className="flex items-center justify-between px-1">
        <label className="text-[11px] font-black uppercase tracking-[0.2em] text-[#8fb8ff] group-focus-within:text-[#d4af37] transition-colors flex items-center gap-2">
          <Icon className="w-3.5 h-3.5 text-[#d4af37]" /> {label}
        </label>
        {hint && isEditing && (
          <span className="text-[10px] text-[#8fb8ff] font-semibold">{hint}</span>
        )}
      </div>
      ...
      <div className="relative group/input">
        <input
          ...
          className={`w-full px-5 pr-28 py-4 text-sm font-mono font-semibold rounded-2xl border transition-all duration-200 outline-none ${isEditing
              ? 'bg-[#040f33] border-[#2450b7] text-white placeholder-[#8fb8ff]/60 focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 shadow-inner'
              : 'bg-[#081d5f]/50 border-[#2450b7] text-[#8fb8ff] cursor-not-allowed opacity-80'
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
          {type === 'password' && isEditing && (
            <button
              ...
              className="p-1.5 rounded-lg text-[#8fb8ff] hover:text-white hover:bg-[#123283] transition-colors"
          ...
          {value && type !== 'password' && (
            <button
              ...
              className="p-1.5 rounded-lg text-[#8fb8ff] hover:text-[#d4af37] hover:bg-[#123283] transition-colors flex items-center gap-1 text-xs"
              title="Copy value"
            >
          ...
          {!isEditing && (
            <div className="px-2 py-0.5 rounded-md bg-[#081d5f] border border-[#2450b7] text-[10px] font-mono text-[#8fb8ff] flex items-center gap-1">
              <Lock className="w-3 h-3 text-[#8fb8ff]" /> LOCKED
            </div>
          )}
        </div>
      </div>
...
const ServerCard = ({ serverType = true, Icon, title, showToast }: any) => {
  ...
  return (
    <div className="relative rounded-[2rem] p-6 sm:p-8 border bg-[#081d5f] border-[#2450b7] shadow-[0_30px_80px_rgba(4,15,54,0.3)] transition-all duration-300">
      {/* Top Header Card Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-[#2450b7]">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-[#0c1c59] border border-[#2450b7] flex items-center justify-center text-[#d4af37] shadow-inner shrink-0">
          ...
          <div>
            ...
            {lastUpdated ? (
              <p className="text-xs text-[#8fb8ff] mt-1 flex items-center gap-1.5">
                <Clock className="w-3 h-3 text-[#d4af37]" /> Last synchronized: <span className="text-white font-mono">{new Date(lastUpdated).toLocaleString()}</span>
              </p>
            ) : (
              <p className="text-xs text-[#8fb8ff] mt-1">Gateway master profile</p>
            )}
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-[#0c1c59] hover:bg-[#123283] border border-[#2450b7] text-[#8fb8ff] hover:text-white text-xs font-semibold flex items-center justify-center gap-2 transition-all shrink-0 uppercase tracking-widest"
        >
          {testingConnection ? (
            <>
      ...
      {loadingLocal ? (
        ...
        <form onSubmit={handleSubmitLocal} className="space-y-6">
          ...
          {/* Action Bar */}
          <div className="pt-6 mt-6 border-t border-[#2450b7]">
            {isEditingLocal ? (
              <div className="space-y-4">
                {hasChanges() && (
                  <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs font-bold flex items-center justify-between shadow-inner">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest border transition-all bg-[#040f33] border-[#2450b7] text-[#8fb8ff] hover:bg-[#123283] hover:text-white disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-4 rounded-xl bg-[linear-gradient(135deg,#e0b01d_0%,#c99508_100%)] text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-[#d4af37]/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50"
                  >
              ...
              <button
                type="button"
                onClick={handleEditToggleLocal}
                className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest border transition-all bg-[#040f33] border-[#2450b7] text-[#d4af37] hover:bg-[#123283] hover:text-white shadow-md"
              >
...
export default function AdminSettingsPage() {
  ...
  if (superuserCheckDone && !isSuperuserUser) {
    return (
      <div className="w-full min-h-screen flex items-center justify-center p-4 bg-[#0c1c59] text-white">
        <div className="max-w-md w-full p-10 rounded-[2.5rem] text-center border transition-all bg-[#081d5f] border-red-500/20 shadow-2xl">
          ...
          <h1 className="text-3xl font-black tracking-tighter mb-4 text-white uppercase">Access Restricted</h1>
          <p className="text-sm font-medium leading-relaxed mb-10 text-[#8fb8ff]">
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen bg-[#0c1c59] text-white font-sans antialiased overflow-y-auto">
        ...
        <div className="max-w-6xl mx-auto p-3 sm:p-4 md:p-8 relative z-10 space-y-6 md:space-y-8">

          {/* Header Banner */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 sm:p-8 rounded-[2rem] bg-[#081d5f] border border-[#2450b7] shadow-[0_20px_60px_rgba(4,15,54,0.2)]">
            <div className="flex items-center gap-5">
              <div className="w-16 h-16 rounded-2xl bg-[#0c1c59] border border-[#2450b7] flex items-center justify-center text-[#d4af37] shadow-inner shrink-0">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-[#8fb8ff] mt-1">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all bg-[#0c1c59] hover:bg-[#123283] text-[#8fb8ff] hover:text-white border border-[#2450b7] flex items-center gap-2 shrink-0 shadow-md"
              >
          ...
          {showGuidelines && (
            <div className="p-6 sm:p-8 rounded-[2rem] border border-[#2450b7] bg-[#081d5f] animate-in slide-in-from-top-4 duration-300 shadow-xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#8fb8ff]">Compliance standards for MT5 Manager API gateway</p>
                </div>
              </div>
              {/* Quick System Status Metrics Bar */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-2xl bg-[#0c1c59] border border-[#2450b7] flex items-center gap-3 shadow-inner">
                  <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400">
                    <Activity className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-[#8fb8ff]">Gateway Status</div>
                ...
                <div className="p-4 rounded-2xl bg-[#0c1c59] border border-[#2450b7] flex items-center gap-3 shadow-inner">
                  ...
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-[#8fb8ff]">Credential Storage</div>
                    <div className="text-sm font-bold text-white">AES-256 ENCRYPTED</div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-[#0c1c59] border border-[#2450b7] flex items-center gap-3 shadow-inner">
                  ...
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-[#8fb8ff]">MT5 API Engine</div>
                    <div className="text-sm font-bold text-white">NATIVE C++ WRAPPER</div>
                  </div>
                </div>
                
                <div className="p-4 rounded-2xl bg-[#0c1c59] border border-[#2450b7] space-y-1 shadow-inner">
                  <div className="text-xs font-bold text-[#d4af37] flex items-center gap-1.5">
                    <ShieldCheck className="w-4 h-4" /> Manager Privileges
                  </div>
                  <p className="text-xs text-[#8fb8ff] leading-relaxed">
                ...
                <div className="p-4 rounded-2xl bg-[#0c1c59] border border-[#2450b7] space-y-1 shadow-inner">
                  ...
                  <p className="text-xs text-[#8fb8ff] leading-relaxed">
                    Ensure server IP <span className="font-mono text-white">185.28.255.35</span> is whitelisted on port <span className="font-mono text-white">443</span>.
                  </p>
                </div>
